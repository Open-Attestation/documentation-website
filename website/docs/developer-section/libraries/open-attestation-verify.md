...
- Is the document issuer identity valid ? (see [identity proof](/docs/docs-section/how-does-it-work/issuance-identity))

A wrapped document (shown below) created using [Open Attestation](/docs/developer-section/libraries/open-attestation) would be required.
...
Refer to [verification methods](/docs/developer-section/libraries/open-attestation-verify#custom-verification) to find out more on how to create your own custom verifier.
...
- `isValid(fragments, ["ISSUER_IDENTITY"])` returns false because there is no [DNS-TXT record](/docs/integrator-section/verifiable-document/ethereum/dns-proof) associated with the Ethereum main network's document store.
