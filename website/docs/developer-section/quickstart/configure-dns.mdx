...
While [the links to guides](#additional-note-for-adding-dns-txt-records) provide the steps on adding DNS `TXT` records for common domain registrars and DNS providers, the steps below show a generic procedure for any DNS provider:

1. Select a domain name that you want to associate with your documents. 

    The domain can either be the root domain (e.g. `openattestation.com`) or a subdomain (e.g. `issuer.openattestation.com`). However, it is recommended to use the root domain, as it will be easier for the viewers of your documents to recognize the domain name visually.
...
3. Prepend the document store address with `addr`. 
    ...
    For more information on switching to the production mode, refer to [this section](#additional-note-for-the-identity-proof-in-production).
...
Below is a list of guides for several domain registrars and DNS providers:
