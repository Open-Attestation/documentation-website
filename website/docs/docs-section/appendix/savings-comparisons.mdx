...
import { TriangleDown, TriangleUp } from "../../../src/components/Icon";
...
export const PriceTable = ({ type, chain, priceFormatOptions }) => {
      ...
      <tr key={idx}>
        ...
        <td align="center">
          ...
          {!v3Gas ? null : savings <= 0 ? (
            <TriangleDown color="#22c55e" style={{ marginLeft: "8px" }} />
          ) : (
            <TriangleUp color="#f43f5e" style={{ marginLeft: "8px" }} />
          )}
        </td>
  ...
  return (
    <div>
      <p>
        Estimations based on the current gas <em>average</em> at <BoxTag>{Math.ceil(gwei)} gwei</BoxTag> and ETH price
        at USD{" "}
        <BoxTag>
